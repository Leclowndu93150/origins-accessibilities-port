...
public abstract class AirCurrentMixin {
    ...
    @Shadow public abstract FanProcessingType getTypeAt(float offset);
    ...
    private Entity oriacs$applyFreshAirEffect(Entity entity) {
        if(entity.tickCount % 20 == 0 && entity instanceof LivingEntity living) {
            FanProcessingType type = getTypeAt((float) entity.position().distanceTo(VecHelper.getCenterOf(this.source.getAirCurrentPos())) - 0.5F);
